...
% PGFPlots
\usepackage{pgfplots}
\pgfplotsset{width=6cm,compat=1.16} %{compat=yourversion}
...
Inconvenientes: es básico y limitado.
\hfill
\href{https://es.overleaf.com/learn/latex/Picture_environment}{\color{blue}Picture environment (Overleaf)}
...
\end{minted}
\vspace{1cm}
\href{https://www.overleaf.com/learn/latex/LaTeX_Graphics_using_TikZ%3A_A_Tutorial_for_Beginners_(Part_1)%E2%80%94Basic_Drawing}{\color{blue}LaTeX Graphics using TikZ (Overleaf)}\\
...
\draw [blue, domain=0:pi]  plot (\x, {sin(\x r)*exp(\x/(2*pi))});
\foreach \x in {0,1,2,3,4}
   \draw (pi /4. * \x  cm,1pt) -- (pi / 4.* \x  cm,-1pt) 
   node[anchor=north] {$\frac{\x \pi}{4}$};
\end{tikzpicture}
\caption{Gr\'afico TikZ}
\end{wrapfigure}
$ $
\vspace{-1.5cm }
\begin{minted}{latex}
...
plot (\x, {sin(\x r)*exp(\x/(2*pi))});
\foreach \x in {0,1,2,3,4}
   \draw (pi /4. * \x cm,1pt) -- (pi /4. * \x cm,-1pt) 
   node[anchor=north] {$\frac{\x \pi}{4}$};
\end{tikzpicture}
\end{minted}
% anchor=north se usa para indicar que el nuevo nodo tenga en su norte al último 
% punto donde se ha hecho algo. 
% Ver  https://tikz.dev/tikz-shapes#sec-17.5.1
%\vspace{2cm}
%\href{https://cremeronline.com/LaTeX/minimaltikz.pdf}{\color{blue}A very minimal introduction to TikZ}\\
%\href{https://es.wikibooks.org/wiki/Manual_de_LaTeX/Inclusi%C3%B3n_de_gr%C3%A1ficos/Gr%C3%A1ficos_con_TikZ}{\color{blue}Manual de LaTeX/Inclusión de gráficos/Gráficos con TikZ}
\end{frame}
...
\href{https://www.overleaf.com/learn/latex/Pgfplots_package}{\color{blue} Pgfplots package (Overleaf)}
...
\begin{frame}[fragile]{Generaci\'on de diagramas de barras con PGFPlots}
\begin{figure}
\begin{tikzpicture}
\begin{axis}[
	x tick label style={
...
\begin{frame}[fragile]{Exportar a \LaTeX{} desde un programa externo}
...
\item \href{https://www.overleaf.com/learn/latex/LaTeX_Graphics_using_TikZ%3A_A_Tutorial_for_Beginners_(Part_2)%E2%80%94Generating_TikZ_Code_from_GeoGebra}{\color{blue} GeoGebra}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Exportar a TikZ desde Geogebra}
...
Nota: Ofrece la posibilidad de exportar el grafico
que acabamos de realizar (puede dar errores no sencillos de detectar/corregir).
%\end{exampleblock}

\href{https://www.overleaf.com/learn/latex/LaTeX_Graphics_using_TikZ%3A_A_Tutorial_for_Beginners_(Part_2)%E2%80%94Generating_TikZ_Code_from_GeoGebra}{\color{blue} Generating TikZ Code from Geogebra (Overleaf)}
...
\href{https://es.wikibooks.org/wiki/Manual_de_LaTeX/Inclusi%C3%B3n_de_gr%C3%A1ficos/Gr%C3%A1ficos_con_TikZ}{\color{blue}Wikibooks: Gráficos con Tikz}
